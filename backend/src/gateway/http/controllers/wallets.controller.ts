...
export class WalletsController {
  ...
  @Get(':userUuid')
  @ApiOperation({ summary: 'Get a wallet by user uuid' })
  @ApiParam({ name: 'userUuid', format: 'uuid', example: 'df030de8-6479-4837-a03d-65836fa80d60' })
  @ApiOkResponse({ description: 'Wallet for the requested user.', type: WalletResponse })
  @ApiBadRequestResponse({ description: 'userUuid must be a UUID.', type: ErrorResponse })
  @ApiUnauthorizedResponse({ description: 'Missing or invalid JWT.', type: ErrorResponse })
  @ApiForbiddenResponse({ description: 'Requires wallets.read permission.', type: ErrorResponse })
  @ApiNotFoundResponse({ description: 'Wallet not found.', type: ErrorResponse })
  @Permissions('wallets.read')
  findByUserUuid(@Param('userUuid', ParseUUIDPipe) userUuid: string) {
    return this.walletsService.findByUserUuid(userUuid);
  }

  @Patch(':userUuid/balance')
  @ApiOperation({ summary: 'Update a wallet balance' })
  @ApiParam({ name: 'userUuid', format: 'uuid', example: 'df030de8-6479-4837-a03d-65836fa80d60' })
  @ApiOkResponse({ description: 'Updated wallet.', type: WalletResponse })
  @ApiBadRequestResponse({ description: 'Invalid userUuid or request body.', type: ErrorResponse })
  @ApiUnauthorizedResponse({ description: 'Missing or invalid JWT.', type: ErrorResponse })
  @ApiForbiddenResponse({ description: 'Requires wallets.update permission.', type: ErrorResponse })
  @ApiNotFoundResponse({ description: 'Wallet not found.', type: ErrorResponse })
  @Permissions('wallets.update')
  updateBalance(@Param('userUuid', ParseUUIDPipe) userUuid: string, @Body() dto: UpdateWalletBalanceRequest) {
    return this.walletsService.updateBalanceByUserUuid(userUuid, dto.balance);
  }
}
